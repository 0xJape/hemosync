...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = db.prepare(`SELECT p.full_name AS patientName, m.heart_rate_bpm AS heartRate, m.spo2_percent AS spo2, m.signal_quality AS signalQuality, m.valid_window_count AS validWindowCount, m.sample_window_count AS sampleWindowCount FROM screening_sessions s JOIN patients p ON p.id = s.patient_id JOIN measurements m ON m.screening_session_id = s.id WHERE s.id = ? AND s.status = 'completed'`).get(id) as { patientName: string; heartRate: number; spo2: number; signalQuality: string; validWindowCount: number; sampleWindowCount: number } | undefined;
  if (!row) return Response.json({ error: "Completed screening result not found." }, { status: 404 });

  const prompt = `You provide cautious, precise patient-facing screening interpretation. Use only supplied values. Never diagnose, name diseases, prescribe treatment, or claim an emergency. Explain what each reported value indicates for this screening: heart rate ${row.heartRate} BPM, SpO2 ${row.spo2}%, and signal quality ${row.signalQuality}. Compare heart rate with a commonly used adult resting screening reference of 60–100 BPM and SpO2 with a commonly used screening reference of 95% or higher, clearly state when a value is below, within, or above that reference. State that these references may not apply during exercise, in children, or with clinical conditions. Poor signal quality or limited valid windows must reduce confidence and recommend repeat measurement. Return JSON only with keys summary and suggestions. summary: 3–4 short sentences, name every available value, explain any value outside reference, and state clear follow-up urgency without emergency claims. suggestions: array of exactly 3 short practical next steps. End summary with: ${disclaimer}\n\nData: heart rate ${row.heartRate} BPM; SpO2 ${row.spo2}%; signal quality ${row.signalQuality}; valid windows ${row.validWindowCount}/${row.sampleWindowCount}.`;
  try {
    ...
    const spokenText = `Scan completed. Average heart rate ${row.heartRate} beats per minute. Blood oxygen ${row.spo2} percent. ${summary} Suggestions: ${suggestions.join(". ")}`.slice(0, 3500);

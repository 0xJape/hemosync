...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const sessions = db.prepare(`SELECT s.id, s.status, s.started_at AS startedAt, s.completed_at AS completedAt, s.cancelled_at AS cancelledAt, m.heart_rate_bpm AS heartRate, m.spo2_percent AS spo2, m.signal_quality AS signalQuality FROM screening_sessions s LEFT JOIN measurements m ON m.screening_session_id = s.id WHERE s.patient_id = ? ORDER BY s.started_at DESC`).all(id);
